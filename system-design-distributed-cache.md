...
flowchart TB
    ...
    subgraph platform["Distributed Cache"]
        edge["WAF / API Gateway / TLS / Auth / Rate Limit"]
        lb["Load Balancer"]
        svc0["Cache Proxy"]
        svc1["Cache Manager"]
        svc2["Replication Svc"]
        store0[("Redis Cluster")]
        store1[("Consistent Hashing")]
        store2[("Raft Consensus")]
        stream{{"Kafka"}}
        worker0["Eviction Workers"]
        worker1["Analytics"]
        worker2["Replication Workers"]
