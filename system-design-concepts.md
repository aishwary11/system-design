...
It matches on frequencies, never on audio. The clip is sampled at 44.1 kHz and run through a sliding **FFT** — 4096-sample windows, 50% overlap, one spectrum frame every ~46 ms. Most of each frame is thrown away: only local energy **peaks** survive, restricted to six log-spaced frequency bands (~40–80, 80–160, 160–320, 320–640, 640–1320, 1320–5512 Hz) because phone speakers and AAC codecs destroy everything outside that range. Each peak is a `(time, frequency)` point; pairs of peaks within a ~100 ms target zone are comb-hashed into one 32-bit integer — `(f1, f2, Δt)` — the **constellation hash**. Offline, every catalog track is fingerprinted into a giant map `hash → [(track_id, t_anchor)]`: an 8M-track catalog yields ~10¹¹ entries (~1 TB at 10 B/entry), sharded by hash across a KV store. At query time the noisy clip produces the same hashes — noise moves amplitudes but rarely *moves peaks*, so most query hashes still hit. Each hit votes for `(track_id, t_db − t_query)`; the true song's votes pile up at one consistent offset while every other track scatters single-digit votes. 2 s of audio ≈ 500–1,000 hashes → one MGET round-trip → histogram. Total match latency ~200 ms, O(1) lookups instead of any audio comparison.
...
Two fingerprint pipelines run on every upload and every rights-holder reference. **Audio**: the Q1 FFT-peak → pair-hash pipeline. **Video**: keyframes sampled at ~1–2 fps are hashed with **perceptual hashes** — 64-bit DCT/block-mean signatures that quantize low frequencies only, so re-encoding, scaling, and mild crops leave them intact. Matching is an inverted-index similarity join: each upload hash fetches candidate references (multi-index hashing over the pHash space), candidates are verified by **Hamming distance ≤ 10**, then the audio hashes confirm the exact time alignment. On a confirmed match the *policy* engine executes the rights holder's rule — block, track views, or redirect monetization — keyed on match percentage and overlap duration. Because everything matches on fingerprints, per-upload cost is independent of video length or resolution.

**Q4. How does face unlock work with no internet connection?**

It's a 1:1 vector match that never leaves the phone. **Enrollment**: the camera frame passes through an on-device CNN (MobileNet-class, trained with triplet loss) that outputs a **128-dimensional embedding** — geometry such that "same face" lands within a small L2 ball. The embedding is stored inside the secure enclave; the image is discarded. **Unlock**: the same network embeds the presented face, and the entire match is one L2 distance computation against the stored template(s), thresholded (~0.6 in FaceNet-style systems). No network, no index — the "database" is 1–5 vectors. Liveness comes from the IR/depth camera (a printed photo has no depth map) plus randomized challenges (blink, turn), and the enclave **signs the verdict** so the OS kernel trusts the result without trusting the camera stack.
...
Inference happens at the edge. An induction loop or radar fires the capture; a frame burst goes straight into the camera's NPU pipeline: an object detector localizes the plate, a **homography** rectifies the perspective, characters are segmented and classified by a small CNN — all within one frame interval at 30 fps. Before anything is transmitted, the plate string is **validated structurally**: every jurisdiction encodes region/vehicle-age in the plate format (regex) and many include check digits, so OCR noise is rejected locally. Only high-confidence events — plate text, cropped image, timestamp, lane, speed — are uploaded (kilobytes per car, not video): the uplink, not compute, is the scarce resource. Watchlist matching runs server-side; the camera is a filter that turns 30 fps of traffic into a few KB of events.

**Q7. How does Google search the entire web in under 200 ms?**

The work happened before your query. The crawler builds an **inverted index**: per term, a sorted posting list of doc-IDs with positions and weights. Query execution is a DAAT/**WAND** traversal — intersect posting lists by doc-ID using skip pointers, and prune any document whose *maximum possible* score can't beat the current top-k heap (WAND skips 90%+ of scoring work). Ranking is two-phase: a cheap scorer over hundreds of precomputed signals (static quality, anchor text, freshness) picks ~1,000 survivors from millions of candidates, then a heavier model re-ranks only those. The index is **tiered** — the hot top-of-web tier sits in RAM, the long tail on disk — and popular queries hit a result cache. <200 ms is: intersect a few short in-RAM lists, prune, re-rank 1K, render.
