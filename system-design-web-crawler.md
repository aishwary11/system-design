...
```mermaid
flowchart TB
    clients["Web UI / API Clients"] --> edge["WAF / API Gateway / TLS / Auth / Rate Limit"]
    edge --> lb["Load Balancer (ALB)"]
    lb --> svc0["Crawler Svc"]
    lb --> svc1["Parser Service"]
    lb --> svc2["Index Service"]
    svc0 --> store0["Redis URL Queue"]
    svc1 --> store1["Cheerio + JSdom"]
    svc2 --> store2["Elasticsearch"]
    store0 --> stream["Kafka"]
    stream --> worker0["Fetch Workers"]
    stream --> worker1["Analytics"]
    stream --> worker2["Dedup Workers"]
