...
```mermaid
flowchart TB
    clients["Web / Mobile / Admin Panel"] --> edge["WAF / API Gateway / TLS / Auth / Rate Limit"]
    edge --> lb["Load Balancer (ALB)"]
    lb --> svc0["Product Svc"]
    lb --> svc1["Cart Service"]
    lb --> svc2["Order Service"]
    svc0 --> store0["PostgreSQL + Redis"]
    svc1 --> store1["Redis + DynamoDB"]
    svc2 --> store2["PostgreSQL + Kafka"]
    store0 --> stream["Kafka"]
    stream --> worker0["Order Workers"]
